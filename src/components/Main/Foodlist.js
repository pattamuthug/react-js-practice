...
function Foodlist(props) {
    ...
    
    {
            props.details.map((items)=>{
                return <Fooditems key={Math.random()} name={items.name} time={items.time} location={items.location} rate={items.rate} img={items.result} id={items.id} items={items}></Fooditems>
            })
     }
    </div>
    );
